...
- **Приближение к лимиту**: уведомление при приближении к лимиту на заданный процент (настраивается через `limit.monitor.approach.threshold`, по умолчанию 1%)
- **Достижение лимита**: уведомление при достижении лимита
- **Типы лимитов**: UP (верхний) и DOWN (нижний)
- **Настраиваемый порог**: порог приближения можно настроить через конфигурацию приложения
...
4. Проверка условий для отправки уведомления:
   - Приближение к лимиту на заданный процент или менее (настраивается через `limit.monitor.approach.threshold`)
   - Достижение лимита (цена равна или превышает лимит)
...
- **`notificationsCache`**: Хранит информацию об отправленных уведомлениях
...
# Порог приближения к лимиту (опционально, можно задать через переменную окружения)
LIMIT_MONITOR_APPROACH_THRESHOLD=0.01  # 1% в десятичном формате
```
...
# Порог приближения к лимиту (в десятичном формате, 0.01 = 1%, 0.05 = 5%)
# Уведомления отправляются когда цена приближается к лимиту на указанный процент или меньше
limit.monitor.approach.threshold=${LIMIT_MONITOR_APPROACH_THRESHOLD:0.01}
...
Порог приближения к лимиту настраивается через свойство `limit.monitor.approach.threshold`:

- **Формат**: десятичное число (0.01 = 1%, 0.05 = 5%, 0.10 = 10%)
- **Значение по умолчанию**: 0.01 (1%)
- **Минимальное значение**: рекомендуется не менее 0.005 (0.5%)
- **Максимальное значение**: рекомендуется не более 0.10 (10%)

**Примеры конфигурации:**

```properties
# 1% (по умолчанию)
limit.monitor.approach.threshold=0.01

# 5%
limit.monitor.approach.threshold=0.05

# 2.5%
limit.monitor.approach.threshold=0.025
```
...
LIMIT_MONITOR_APPROACH_THRESHOLD=0.05
```

При инициализации сервиса в логах будет выведена информация о настроенном пороге:

```
===============================================================
🔧 Инициализация LimitMonitorService
📊 ПОРОГ ПРИБЛИЖЕНИЯ К ЛИМИТУ: 5.00% (десятичное значение: 0.05)
   Уведомления будут отправляться при приближении к лимиту на 5.00% и менее
   Настройка: limit.monitor.approach.threshold=0.05
===============================================================
```
...
### Логирование порога приближения

При инициализации сервиса и при всех важных событиях в логах отображается настроенный порог приближения:

**При инициализации:**
```
===============================================================
🔧 Инициализация LimitMonitorService
📊 ПОРОГ ПРИБЛИЖЕНИЯ К ЛИМИТУ: 1.00% (десятичное значение: 0.01)
   Уведомления будут отправляться при приближении к лимиту на 1.00% и менее
   Настройка: limit.monitor.approach.threshold=0.01
===============================================================
```

**При проверке лимита (DEBUG):**
```
Проверка лимита для SBER (UP): текущая цена=250.50, лимит=255.00, расстояние=1.80%, порог приближения=1.00%
```

**При достижении лимита (INFO):**
```
🚨 Лимит UP достигнут для SBER (BBG004730N88): цена=255.00, лимит=255.00, порог приближения=1.00%
```

**При приближении к лимиту (INFO):**
```
⚠️ Приближение к лимиту UP для SBER (BBG004730N88): расстояние=0.80%, порог=1.00%
```

**При отправке уведомления (INFO):**
```
📊 Данные уведомления:
   - Тикер: SBER
   - FIGI: BBG004730N88
   ...
   - Текущая цена: 252.50 ₽
   - Цена лимита: 255.00 ₽
   - ПОРОГ ПРИБЛИЖЕНИЯ: 1.00% (настроен в limit.monitor.approach.threshold)
```
